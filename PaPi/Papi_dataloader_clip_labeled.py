...
class CIFAR100Partialize(Dataset):
    def __init__(self, X, Y, num_classes):
        ...
        for i in range(len(self.X)):
            if self.YT[i] == 1:
                self.given_partial_label_matrix[i][self.Y[i]] = 1.0
            else:
                self.given_partial_label_matrix[i][:] = 1.0
                self.given_partial_label_matrix[i][self.Y[i]] = 0.0
...
class DatasetPartialize(Dataset):
    def __init__(self, X, Y, dataset_name, num_classes):
        self.X = X
...
def get_data_handler(dataset):
    if dataset == 'CIFAR100':
        ...
        datahandler = DatasetPartialize(train_data, train_label, dataset_name=dataset, num_classes=num_classes)
    return datahandler
