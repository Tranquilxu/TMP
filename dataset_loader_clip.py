...
class CIFAR100_handler_train_clip(Dataset):
    def __init__(self, X, Y, input_size, transform=None):
        ...
        for i in range(len(self.X)):
            ...
            file.write(str(r) + '\n')
            file.close()
        print("correct_num = ", self.correct_num)
        print("x_num = ", len(self.X))
        print("acc = ", self.correct_num / len(self.X))
...
def get_data_handler(dataset, pattern, input_size):
    if dataset == 'CIFAR100':
        ...
        if pattern == "train":
            datahandler = CIFAR100_handler_train_gened_clip(train_data, train_label, input_size)
        elif pattern == "val":
